...
{
    int len = strnlen(filename, SERPAEOS_MAX_PATH);
...
{
    char* result_path_part = kzalloc(SERPAEOS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if (strlen(path) > SERPAEOS_MAX_PATH)
    {
        goto out;
    }
...
struct path_part *pathparser_get_last_part(struct path_part *path)
{
    struct path_part *curr = path;
    while(curr->next)
        curr = curr->next;
    return curr;
}