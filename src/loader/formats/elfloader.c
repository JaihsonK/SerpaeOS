...
{
    return header->e_type == ET_EXEC && header->e_entry >= SERPAEOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
{
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? SERPAEOS_ALL_OK : -EINFORMAT;
}
...
int elf_load(const char* filename, struct elf_file** file_out)
{
    ...
    res = fread(fd, elf_file->elf_memory, stat.filesize);
    if (res < 0)
    {
        goto out;
    }
